...
<script>
  export default {
    props: {
      mapcenter: {
        type: String,
        default: '40.0024431613261,116.3263320'
      },
      oldmarker: {
        type: String,
        default: '40.0024431613261,116.3263320'
    ...
    watch: {
      ...
      inputVal(newVal, oldVal){
        this.$emit('mapclick', {text:this.inputVal});
      }
    },
    methods: {
      loadQQmap() {
        ...
        //script.src = "https://map.qq.com/api/js?v=2.exp&callback=onMapFileLoad&key=0";
        script.src =   "https://map.qq.com/api/js?v=2.exp&key=S3YBZ-GFP36-QGJSK-MEV46-I66IS-QQFRJ&callback=onMapFileLoad";
        //添加标签到dom
      ...
      bindMapEvent() {
        ...
        qq.maps.event.addListener(that.qqmap, 'click', function (event) {
          ...
          that.$emit('mapclick', {pos:event.latLng,text:that.inputVal}); // 地图点击坐标 传递给父组件
